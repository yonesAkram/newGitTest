<?php    
	...
	function checkUserStatus($user){
		...
		GLOBAL $con;
        $stmtRg = $con->prepare("SELECT `Username`,`RegStatus` FROM `users` WHERE `Username`=?AND`RegStatus`= 0");
        $stmtRg->execute(array($user));
        $Status = $stmtRg->rowCount();
        return $Status;
...
/********************************************************************* 
	...
	** Function To Check Table `Users`  From DataBase Check RegStatus Of the User  
/*********************************************************************/ 	
